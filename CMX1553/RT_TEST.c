...
{	
	...
	Handle = CM1553OpenBoard(RS_485_INTERFACE, 4, IP, CMX1553_boardID, &Error);//PORT NO 4
	printf("1553 Openboard error is : %d\n ", Error);
	Sleep(500);

	CM1553BoardSetMode(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, 0, 1, 1, 1, &Error);
	printf("1553 Boardsetmode error is : %d\n ", Error);
	Sleep(500);

	CM1553Rt1SetAddr(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, RT_ADDR15, &Error);
	printf("1553 RT1 set addr error is : %d\n ", Error);
	Sleep(500);

	CM1553ResetIC(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, &Error);
	printf("1553 Reset error is : %d\n ", Error);
	Sleep(500);
	
	CM1553Rt1Enable(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, &Error);
	printf("1553 RT1 enable error is : %d\n ", Error);
	Sleep(500);
	
	CM1553SharedInit(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, &Error);
	printf("1553 SharedInit error is : %d\n ", Error);
	Sleep(500);
	
	CM1553RtCheckAddr(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, RT1, &Error);
	printf("1553 RT Check Addr error is : %d\n ", Error);
	Sleep(500);

	CM1553Rt1Init(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, SUPPORT_BRDCST, UNDEF_MCODES_VALID, USE_SMCP, &Error);
	printf("1553 RT1 init error is : %d\n ", Error);
	Sleep(500);
	CM1553Rt1Init1(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, USE_SMCP, &Error);
	printf("1553 RT1 init1 error is : %d\n ", Error);
	Sleep(500);

	CM1553Rt1Init2(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, ILLEGAL_CMD_DETECT, &Error);
	printf("1553 RT1 init2 error is : %d\n ", Error);
	Sleep(500);
	
	CM1553Rt1SetTXData(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, data7, 26, &Error);
	printf("1553 RT1 Set Tx data error is : %d\n ", Error);
	Sleep(500);

	CM1553Rt1SetTTAG(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, TTAG_64U, &Error);
	printf("1553 RT1 Set TTAG error is : %d\n ", Error);
	Sleep(500);

	CM1553Rt1Start(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, &Error);
	...
	for(int i=0;i<20;i++)
	{
		...
		{
					CM1553RtGetMessage(RS_485_INTERFACE,IP,Handle,CMX1553_boardID,rtmessage,RT1,&Error);
					printf("1553 SharedInit error is : %d\n ", Error);
					Sleep(500);
		}


		CM1553Rt1Stop(RS_485_INTERFACE, IP, Handle, CMX1553_boardID, &Error);
		printf("1553 RT1 Stop error is : %d\n ", Error);
		Sleep(500);
		CM1553DiscardBoard(RS_485_INTERFACE,IP,Handle,CMX1553_boardID,&Error);
